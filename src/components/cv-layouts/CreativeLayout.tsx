...
const CreativeLayout: React.FC<CVLayoutProps> = ({
  data,
  className = ''
}) => {
  const colors = getBrancheColors(data.branche);
  return <div className={`max-w-4xl mx-auto bg-white shadow-2xl overflow-hidden relative ${className}`} data-cv-preview>
      {/* Decorative Elements */}
      ...
      <div className="relative p-8 overflow-hidden">
        <div className="absolute inset-0 transform -skew-y-1" style={{
      ...
      <div className="grid md:grid-cols-5 gap-6 p-8">
        ...
        <div className="md:col-span-2 space-y-6">
          {/* Languages */}
          {data.sprachen && data.sprachen.length > 0 && <div>
              <h3 className={`text-xl font-bold text-[hsl(${colors.text})] mb-4 relative`}>
                Sprachen
                
              </h3>
              <div className="space-y-4">
                {data.sprachen.map((sprache, index) => <div key={index} className={`bg-[hsl(${colors.secondary})] p-3 rounded-lg transform hover:scale-105 transition-transform`}>
          ...
          {/* Skills */}
          {(data.status === 'azubi' || data.status === 'ausgelernt') && data.faehigkeiten && data.faehigkeiten.length > 0 && <div>
              <h3 className={`text-xl font-bold text-[hsl(${colors.text})] mb-4 relative`}>
                Fähigkeiten
        ...
        <div className="md:col-span-3 space-y-6">
          ...
          {/* Education */}
          {data.schulbildung && data.schulbildung.length > 0 && <div>
              <h3 className={`text-xl font-bold text-[hsl(${colors.text})] mb-4 relative`}>
                Schulbildung
                
              </h3>
              <div className="space-y-4">
                {data.schulbildung.map((schule, index) => <div key={index} className={`bg-gradient-to-r from-[hsl(${colors.secondary})] to-white p-4 rounded-lg border-l-4 border-[hsl(${colors.primary})] transform hover:scale-105 transition-transform`}>
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-lg">{schule.schulform}</h4>
                      <span className="text-sm text-white px-3 py-1 rounded-full font-medium" style={{
                  background: `hsl(${colors.primary})`
                }}>
                    ...
                    {schule.beschreibung && <p className="text-sm text-gray-600">{schule.beschreibung}</p>}
                  </div>)}
              </div>
            </div>}

          {/* Work Experience */}
          {data.berufserfahrung && data.berufserfahrung.length > 0 && <div>
              <h3 className={`text-xl font-bold text-[hsl(${colors.text})] mb-4 relative`}>
                Praktische Erfahrung
                
              </h3>
              <div className="space-y-4">
                {data.berufserfahrung.map((arbeit, index) => <div key={index} className={`bg-gradient-to-r from-[hsl(${colors.secondary})] to-white p-4 rounded-lg border-l-4 border-[hsl(${colors.primary})] transform hover:scale-105 transition-transform`}>
                    <div className="flex justify-between items-start mb-2">
                  ...
                  background: `hsl(${colors.primary})`
                    ...
                    {arbeit.beschreibung && <p className="text-sm text-gray-600">{arbeit.beschreibung}</p>}
                  </div>)}
              </div>
            </div>}
        </div>
