-- Matching System Foundation Migration
...
CREATE TABLE company_candidate_suppression (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES companies(id) ON DELETE CASCADE,
    candidate_id UUID REFERENCES candidates(id) ON DELETE CASCADE,
    reason TEXT,
...
CREATE INDEX idx_match_cache_job_id ON match_cache(job_id);
CREATE INDEX idx_match_cache_candidate_id ON match_cache(candidate_id);
CREATE INDEX idx_match_cache_score ON match_cache(score);
...
-- Insert sample data
INSERT INTO companies (name, slug, industry, size_range, description) VALUES
('TechCorp GmbH', 'techcorp', 'Technology', '50-200', 'Innovative technology company');
...
INSERT INTO certs (name, category) VALUES
('Führerschein Klasse B', 'Transport'),
('Erste Hilfe', 'Safety'),
('ISO 9001', 'Quality'),
('AWS Certified', 'Cloud');

-- Create views for easier querying
CREATE VIEW v_jobs_with_company AS
SELECT 
    j.*,
...
CREATE VIEW v_candidate_public AS
SELECT 
    id,
    vorname,
    nachname,
