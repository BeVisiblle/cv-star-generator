  ...
  return (
    <div className="mx-auto max-w-6xl px-4">
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1.2fr),minmax(0,0.8fr)] items-center">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-white/70 shadow-[0_18px_40px_rgba(81,112,255,0.20)] border border-[#e7e7ff]'
                      : 'bg-white/5 text-gray-500 hover:bg-white/40 border border-transparent'
                  )}
                >
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <span className="text-xs uppercase tracking-[0.35em] text-gray-400">{card.subtitle}</span>
                      <h3 className={cn('mt-2 text-lg font-semibold', isActive ? 'text-gray-900' : 'text-gray-600')}>
                        {card.title}
                      </h3>
                    </div>
                    <span
                      className={cn(
                        ...
                        isActive ? 'border-gray-900 text-gray-900' : 'border-gray-300 text-gray-400'
                      )}
                    >
                      {String(idx + 1).padStart(2, '0')}
                    </span>
                  </div>
                  <p className={cn('mt-3 text-sm leading-relaxed', isActive ? 'text-gray-600' : 'text-gray-500/80')}>
                    {card.description}
                  </p>
                </button>
              );
        ...
        {/* Visual */}
        <div className="relative overflow-hidden rounded-[32px] bg-white/90 shadow-[0_24px_50px_rgba(81,112,255,0.15)] max-h-[430px]">
          ...
          <div className="absolute bottom-4 right-4">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-full bg-white/90 px-5 py-2 text-sm font-semibold text-gray-900 shadow-md transition hover:bg-white"
            >
              {activeCard.cta || 'Mehr erfahren'}
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-900 text-white text-xs">→</span>
            </Link>
          </div>
        </div>
      </div>
