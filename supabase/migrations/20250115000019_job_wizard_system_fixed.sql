-- Job Wizard System Migration - FIXED VERSION
-- Uses OR REPLACE to avoid conflicts
...
DROP TRIGGER IF EXISTS trg_set_job_quality ON jobs;
...
DROP POLICY IF EXISTS "Users can manage their own job drafts" ON job_drafts;
CREATE POLICY "Users can manage their own job drafts" ON job_drafts
    FOR ALL USING (user_id = current_user_id());

-- Apply updated_at trigger to job_drafts
DROP TRIGGER IF EXISTS set_updated_at_job_drafts ON job_drafts;
