-- AI Matching System - Final Migration with Vector Embeddings
-- Complete system with AI matching capabilities

-- Create candidates table for matching system with AI embeddings
...
-- Create jobs table for matching system with AI embeddings
...
-- Create access_grants table for profile unlocking
CREATE TABLE IF NOT EXISTS public.access_grants (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  job_id UUID REFERENCES public.jobs(id) ON DELETE CASCADE,
  candidate_id UUID REFERENCES public.candidates(id) ON DELETE CASCADE,
  scope TEXT NOT NULL DEFAULT 'profile_view',
...
CREATE POLICY "Companies can manage access grants" ON public.access_grants
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.jobs j
      JOIN public.company_users cu ON cu.company_id = j.company_id
      WHERE j.id = access_grants.job_id 
      AND cu.user_id = auth.uid()
    )
  );
...
LEFT JOIN public.access_grants ag ON mc.candidate_id = ag.candidate_id AND mc.job_id = ag.job_id AND ag.scope = 'profile_view'
WHERE mc.job_id IN (
...
BEGIN
    ...
    INSERT INTO public.access_grants (job_id, candidate_id, scope, granted_by)
    VALUES (p_job_id, p_candidate_id, 'profile_view', auth.uid())
