  ...
  return <header className="sticky top-0 z-40 h-14 flex items-center border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 px-3 sm:px-4">
      <div className="flex items-center gap-2 w-full">
        ...
        <div className="relative w-44 sm:w-64 lg:w-72 mx-2 hidden sm:block">
          ...
          <Input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} onFocus={() => setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 120)} placeholder={(() => {
        ...
        })()} className="pl-10 h-9" aria-label="Globale Suche" />
          <SearchAutosuggest query={q} open={open && !!q} onSelect={(type, payload) => {
          setOpen(false);
          if (type === "person") {
