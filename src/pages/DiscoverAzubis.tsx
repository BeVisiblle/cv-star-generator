...
const DiscoverAzubis: React.FC = () => {
  ...
  const [results, setResults] = useState<CandidateDistance[] | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('search_candidates_within_radius', {
        lat_input: loc.lat,
        lon_input: loc.lon,
        radius_km: Number(radius)
      });
      if (error) throw error;
      setResults(data as CandidateDistance[]);
    } catch (e) {
      console.error('Radius-Suche Fehler (Azubis):', e);
      setResults([]);
    } finally {
      setLoading(false);
    }
        ...
        {results ? (
          results.length ? (
            <div className="space-y-3">
              {results.map((r) => (
                <div key={r.profile_id} className="p-3 rounded-md border bg-muted/30 flex items-center justify-between">
                  <div>
                    <div className="font-medium">{r.city || 'Unbekannt'}</div>
                    <div className="text-sm text-muted-foreground">PLZ {r.postal_code || plz}</div>
                  </div>
